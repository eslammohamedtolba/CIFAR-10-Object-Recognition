...
# Load the model
model = load_model('./preparingmodel\CNNModel.h5')
...
def predict():
    ...
    probability_result = model.predict(np.expand_dims(resized_image, axis=0))
    # Process the prediction result
    predicted_class = classes[np.argmax(probability_result)]
